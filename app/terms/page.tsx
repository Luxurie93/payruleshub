export default function Page() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="prose prose-sm max-w-none">
        <h1>Terms of Service</h1>
        <p>
          <strong>Last updated:</strong> June 2026
        </p>

        <h2>1. Acceptance of Terms</h2>
        <p>
          By accessing and using PayCalc (including all calculators and content),
          you agree to these terms. If you disagree, do not use this site.
        </p>

        <h2>2. Use License</h2>
        <p>
          Permission is granted to temporarily download one copy of the
          materials (information or software) on PayCalc for personal,
          non-commercial transitory viewing only.
        </p>

        <h2>3. Disclaimer of Warranties</h2>
        <p>
          <strong>The materials and calculators are provided on an "as-is"
          basis.</strong> We make no warranties, expressed or implied, including
          accuracy, completeness, or fitness for a particular purpose.
        </p>

        <h2>4. Limitation of Liability</h2>
        <p>
          In no event shall PayCalc or our suppliers be liable for any damages
          (including lost profits, data, or business interruption) arising from
          the use or inability to use the materials or calculators.
        </p>

        <h2>5. Not Professional Advice</h2>
        <p>
          PayCalc provides calculators for informational purposes only. We are
          not licensed tax advisors, accountants, or employment attorneys. Do
          not rely on our calculators for legal, tax, or professional business
          decisions. Consult qualified professionals.
        </p>

        <h2>6. Accuracy and Errors</h2>
        <p>
          While we strive for accuracy, our calculators are estimates based on
          standard formulas. Results may not reflect your specific situation due
          to variations in tax law, employer policies, and state regulations.
        </p>

        <h2>7. Prohibited Use</h2>
        <p>You may not:</p>
        <ul>
          <li>Use automated tools to scrape or bulk-download our calculators</li>
          <li>Reverse-engineer or attempt to access our source code</li>
          <li>Conduct illegal activities or violate local, state, or federal laws</li>
          <li>Harass, abuse, or threaten our team or other users</li>
          <li>Violate intellectual property rights</li>
        </ul>

        <h2>8. Third-Party Links</h2>
        <p>
          PayCalc may contain links to third-party sites. We are not responsible
          for their content, accuracy, or privacy practices.
        </p>

        <h2>9. Intellectual Property</h2>
        <p>
          All content, calculators, code, and design on this site are owned by
          PayCalc or our content providers. You may not reproduce, modify, or
          redistribute without explicit permission.
        </p>

        <h2>10. Changes to Terms</h2>
        <p>
          We reserve the right to modify these terms at any time. Continued use
          of the site constitutes acceptance of updated terms.
        </p>

        <h2>11. Governing Law</h2>
        <p>
          These terms are governed by applicable U.S. federal and state laws.
        </p>

        <h2>12. Contact</h2>
        <p>
          For terms questions, contact us at <strong>support@example.com</strong>
        </p>
      </div>
    </div>
  );
}
